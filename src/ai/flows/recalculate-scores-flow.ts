...
| 👍 잘한 점 (Strengths) | 💡 개선점 (Areas for Improvement) |
| --- | --- |
| [{{#if assessmentType === 'dialogue'}}AI의 질문을 잘 이해하고 적절하게 답변한 부분을 목록 형식(-)으로 칭찬해 주세요.{{else}}- 혼자 말하기 과제에서는 평가하지 않는 항목입니다.{{/if}}] | [{{#if assessmentType === 'dialogue'}}상호작용에서 아쉬웠던 점이나 개선할 점을 목록 형식(-)으로 지적해 주세요.{{else}}- 혼자 말하기 과제에서는 평가하지 않는 항목입니다.{{/if}}] |
`,
});
