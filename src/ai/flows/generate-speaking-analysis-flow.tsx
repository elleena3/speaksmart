...
// The output schema is changed to return all analysis data directly to the client.
const GenerateSpeakingAnalysisOutputSchema = z.object({
 ...
 * It orchestrates the entire analysis process and returns the results.
 * Firestore operations are now handled on the client-side.
 */
export async function generateSpeakingAnalysis(
    input: GenerateSpeakingAnalysisInput
): Promise<z.infer<typeof GenerateSpeakingAnalysisOutputSchema>> {
  ...
  async (input) => {
    ...
    const [contentResult, pronunciationResult] = await Promise.all([
      contentAnalysisPrompt({
        studentTranscript: input.studentTranscript || studentTranscript, // For dialogue, use the full conversation
      ...
      pronunciationAnalysisPrompt({
        ...
        studentTranscript: studentTranscript, // For pronunciation, use only the student's part
      })
    ]);
    ...
    return {
        studentTranscript: input.studentTranscript || studentTranscript, // Return the full conversation for dialogue, or just transcript for monologue
