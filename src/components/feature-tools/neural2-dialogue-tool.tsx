...
export function Neural2DialogueTool() {
  ...
  const startConversation = async () => {
    ...
    try {
      ...
      toast({
        title: "대화 시작 오류",
        description: "AI와 대화를 시작하는 데 실패했습니다.",
        variant: "destructive",
      });
    ...
    try {
      ...
      mediaRecorderRef.current.onstop = () => {
        if (audioChunksRef.current.length === 0) {
            ...
            toast({ title: "목소리가 들리지 않아요", variant: "destructive" });
            cleanupRecorder();
            return;
        }
      ...
      mediaRecorderRef.current.onerror = (event) => {
          toast({ title: "녹음 오류", variant: "destructive" });
          setInterimTranscript(null);
          setSessionState("waiting_for_user");
          cleanupRecorder();
      };
      ...
      toast({ title: "마이크 접근 오류", variant: "destructive" });
      setSessionState("waiting_for_user");
      setInterimTranscript(null);
      return false;
    }
  ...
  return (
    <div className="flex flex-col gap-4">
          ...
          <div className="space-y-4">
              {sessionState === "idle" && (
                <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground pt-12">
                    ...
                    <p className="font-semibold">AI 'Puck'과 자유롭게 대화해보세요.</p>
              ...
              {sessionState === "initializing" && (
                <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground pt-12">
                    ...
                    <p className="font-semibold">AI 대화 파트너 "Puck"을 연결하는 중입니다...</p>
                </div>
              )}
      ...
      <div className="flex flex-col gap-2">
        ...
        {sessionState !== 'idle' && sessionState !== 'initializing' && sessionState !== 'countdown' && (
