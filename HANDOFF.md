...
5. **세션 쿠키가 기존 로그인에 매끄럽게 붙는지** — 배포 전에 로그인해 둔 브라우저에는
   쿠키가 없습니다. `onAuthStateChanged`에서 자동으로 심도록 해 두었고 로컬에서는 확인했지만,
   실제 배포 환경은 미확인입니다. 도구가 "로그인이 필요합니다"로 막히면 다시 로그인하면 됩니다.
...
- **`GOOGLE_GENAI_API_KEY`를 교체해야 합니다.** 위에 적은 대로 공개 엔드포인트로 노출됐던
  키입니다. 코드는 막았지만 이미 나간 키는 그대로 유효합니다.
