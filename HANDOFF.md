...
- **`ANTHROPIC_API_KEY`가 Vercel에 없습니다.** Claude 모델 선택 시 실패합니다.
  로컬 `.env`에는 있어 검증은 가능합니다. 넣으면 루브릭 파일 분석의 Claude 선택지가 살아납니다.
- **평가 모델로 Claude를 고르면 반드시 실패합니다.** 키와 무관합니다.
  monologue·dialogue 평가는 전사 단계에서 음성을 그 모델로 보내는데 Claude가 오디오를 거부합니다(400).
  실제로 호출해 확인했습니다. 목록에서 빼거나 막는 처리가 아직 없습니다.
...
- `claude-sonnet-5`는 구조화 출력이 불안정합니다(4회 중 2회, 데이터 대신 JSON 스키마를 그대로 반환).
  `withRetry`는 과부하(503)만 재시도하므로 스키마 오류에는 안전망이 없습니다.
